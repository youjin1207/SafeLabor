...
weights = gauss$w; nodes = gauss$x
sourceCpp("Code/Rcpp/rcpp_fun_real.cpp")
negloglike.lognormal.null = loglikeli(main.est[4:length(main.est)])

#### nulliparous (BMI) ####
nullbmi = read.csv("Data/Lognormal_real/null_bmi_Q10.csv", sep=",", header = FALSE)
...
main.est = as.numeric(read.csv("Data/Lognormal_real/noboot_null_bmi_Q10_8169573.csv",
                               sep=",", header = FALSE))
...
negloglike.lognormal.nullbmi = loglikeli(main.est[4:length(main.est)])



##### nulliparous (Age) #####
nullage = read.csv("Data/Lognormal_real/null_age_Q10.csv", sep=",", header = FALSE)
...
negloglike.lognormal.nullage = loglikeli(main.est[4:length(main.est)])

############################################################
#### multiparous (bmi + age) ####
multiresult10 = read.csv("Data/Lognormal_real/multi_without_beyond10.csv", sep=",", header = FALSE)
var.name = c('Seed','n','Runtime', 'over(SVD)', 'obese(SVD)', 'age(SVD)', 'age2(SVD)',
...
d.time = data$obs.T*60; ind = which(d.time > 10)
obs.T = d.time[ind]
...
negloglike.lognormal.multi = loglikeli(main.est[4:length(main.est)])

### multiparous (bmi) ####
multiresult10 = read.csv("Data/Lognormal_real/multi_beyond10_bmi_Q10.csv", sep=",", header = FALSE)
var.name = c('Seed','n','Runtime', 'over(SVD)', 'obese(SVD)', 
...
negloglike.lognormal.multibmi = loglikeli(main.est[4:length(main.est)])

#### multiparous (age) ####
multiresult10 = read.csv("Data/Lognormal_real/multi_beyond10_age_Q10.csv", sep=",", header = FALSE)
var.name = c('Seed','n','Runtime', 'age(SVD)', 'age2(SVD)', 
...
ind = which(d.time > 10); obs.T = d.time[ind]
obs.Delta = data$obs.Delta[ind]; obs.Delta = ifelse(obs.Delta == 3, 2, obs.Delta)
obs.U = data$obs.U[ind]
obs.bmi1 = data$obs.bmi1[ind]; obs.bmi2 = data$obs.bmi2[ind]; obs.bmi3 = data$obs.bmi3[ind]
obs.age = data$obs.age[ind]; obs.age2 = (obs.age)^2
...
negloglike.lognormal.multiage = loglikeli(main.est[4:length(main.est)])