...
sourceCpp("Code/Rcpp/rcpp_weibull.cpp")
...
# ------------------------------- generate data
eta = rweibull(n, gamma, scale = 1)
X = rnorm(n, 0, 1)
W1 = runif(n, 0, 1)

# generate time to event from competing-risks   
d.time = (-log(W1) / (eta*exp(betas[1]*X)*(phis[1])^(tau) + eta*exp(betas[2]*X)*(phis[2])^(tau)))^(1 / tau) 

#generate competing risks data
csh1 <- c(); csh2 <- c(); p <- c(); Delta <- c()
for(i in 1:n){
  csh1[i] = (d.time[i])^(tau - 1)*eta[i]*exp(betas[1]*X[i])*(phis[1]^(tau))*tau
  csh2[i] = (d.time[i])^(tau - 1)*eta[i]*exp(betas[2]*X[i])*(phis[2]^(tau))*tau
...
W2 = runif(n, 0, 1) 
N = (-log(W2) / (psi^(omega)*exp(alpha*X)*eta))^(1/omega) # unobserved onset time of neonatal morbidity 
...
sample.size = length(ind) # this sample size can be less than n (=500), as we cut off the larger T
...
init = rep(0, 31)
gauss = gauss.quad(20, kind="laguerre") # the number of quadrature nodes
...
if(DUMZ==1000655){
  write.table(t(c(DUMZ,n,rectime[3],result)),file=paste('est_weibull15_n500_Q20_',DUMZ,'.csv',sep=''),sep=',',
              col.names=c('Seed','n','Runtime', 'beta1(0.0)', 'beta2(-2.0)', 'alpha(1.5)', 
                          rep('xi1', 9), rep('xi2', 9), rep('phi', 9), 'gamma(1.5)' ),row.names=F)
}else{
  write.table(t(c(DUMZ,n,rectime[3],result)),file=paste('est_weibull15_n500_Q20_',DUMZ,'.csv',sep=''),sep=',',col.names=F,row.names=F)
}
...
if(DUMZ==1000655){
  write.table(t(c(DUMZ,n,rectime[3],mcsd)),file=paste('Qsd_weibull15_n500_Q20_', DUMZ,'.csv',sep=''),sep=',',
              col.names=c('Seed','n','Runtime', 'beta1(1.0)', 'beta2(-2.0)', 'alpha(1.5)', 
                          rep('xi1', 9), rep('xi2', 9), rep('phi', 9), 'gamma(1.5)' ),row.names=F)
  write.table(t(c(DUMZ,n,rectime[3],mcL)), file=paste('QciL_weibull15_n500_Q20_', DUMZ,'.csv',sep=''),sep=',',
              col.names=c('Seed','n','Runtime', 'beta1(1.0)', 'beta2(-2.0)', 'alpha(1.5)', 
                          rep('xi1', 9), rep('xi2', 9), rep('phi', 9), 'gamma(1.5)' ),row.names=F)
  write.table(t(c(DUMZ,n,rectime[3],mcU)),file=paste('QciU_weibull15_n500_Q20_', DUMZ,'.csv',sep=''),sep=',',
              col.names=c('Seed','n','Runtime', 'beta1(1.0)', 'beta2(-2.0)', 'alpha(1.5)',
                          rep('xi1', 9), rep('xi2', 9), rep('phi', 9), 'gamma(1.5)' ),row.names=F)
  write.table(t(c(DUMZ,n,rectime[3],mcavg)),file=paste('Qavg_weibull15_n500_Q20_', DUMZ,'.csv',sep=''),sep=',',
              col.names=c('Seed','n','Runtime', 'beta1(1.0)', 'beta2(-2.0)', 'alpha(1.5)',
                          rep('xi1', 9), rep('xi2', 9), rep('phi', 9), 'gamma(1.5)' ),row.names=F)
}else{
  write.table(t(c(DUMZ,n,rectime[3],mcsd)),file=paste('Qsd_weibull15_n500_Q20_', DUMZ,'.csv',sep=''),sep=',',
