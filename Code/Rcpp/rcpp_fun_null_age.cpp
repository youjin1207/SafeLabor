...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double H = sumC(Isplines.row(ind) * exp(Theta[idx4]));
	  ...
	  double cause1 = pow(exp(-frailty)*lambda1*exp(obsage[ind]*Theta[0] + obsage2[ind]*Theta[1]), obsDelta[ind] == 1); 
  	double cause2 = pow(exp(-frailty)*lambda2*exp(obsage[ind]*Theta[2] + obsage2[ind]*Theta[3]), obsDelta[ind] == 2); 
  	double cause3 = pow(exp(-frailty)*lambda3*exp(obsage[ind]*Theta[4] + obsage2[ind]*Theta[5]), obsDelta[ind] == 3); 


  	double cumcause1 =  exp(-frailty)*Lambda1*exp(obsage[ind]*Theta[0] + obsage2[ind]*Theta[1]); 
  	double cumcause2 =  exp(-frailty)*Lambda2*exp(obsage[ind]*Theta[2] + obsage2[ind]*Theta[3]); 
  	double cumcause3 =  exp(-frailty)*Lambda3*exp(obsage[ind]*Theta[4] + obsage2[ind]*Theta[5]); 

  	double dtimel = cause1*cause2*cause3*exp( -(cumcause1 + cumcause2 + cumcause3));

  	double with = pow( 1 - exp( - exp(-frailty)*H*exp(obsage[ind]*Theta[6] + obsage2[ind]*Theta[7])), obsU[ind] == 1);
  	double without = pow( exp(- exp(-frailty)*H*exp(obsage[ind]*Theta[6] + obsage2[ind]*Theta[7])), obsU[ind] == 0); 
