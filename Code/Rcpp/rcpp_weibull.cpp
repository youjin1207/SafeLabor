...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double cause1 = pow(frailty*lambda1*exp(obsX[ind]*Theta[0]), obsDelta[ind] == 1); 
	double cause2 = pow(frailty*lambda2*exp(obsX[ind]*Theta[1]), obsDelta[ind] == 2); 

	double cumcause1 =  frailty*Lambda1*exp(obsX[ind]*Theta[0]); 
	double cumcause2 =  frailty*Lambda2*exp(obsX[ind]*Theta[1]); 

	double dtimel = cause1*cause2*exp( -(cumcause1 + cumcause2));

	double with = pow( 1 - exp( - frailty*H*exp(obsX[ind]*Theta[2])), obsU[ind] == 1);
	double without = pow( exp(- frailty*H*exp(obsX[ind]*Theta[2])), obsU[ind] == 0); 
	       
	 // current-status data model
	 double morbl = with*without;
	   
	 // individual contribution to the likelihood.
	 double marginal = exp(Theta[30])*pow(frailty, exp(Theta[30])-1)*exp(-pow(frailty,exp(Theta[30])))*exp(frailty);
