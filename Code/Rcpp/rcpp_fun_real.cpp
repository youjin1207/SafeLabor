...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double H = sumC(Isplines.row(ind) * exp(Theta[idx4]));
	  ...
	  double cause1 = pow(exp(-frailty)*lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1] + obsage[ind]*Theta[2] + obsage2[ind]*Theta[3]), obsDelta[ind] == 1); 
  	double cause2 = pow(exp(-frailty)*lambda2*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5] + obsage[ind]*Theta[6] + obsage2[ind]*Theta[7]), obsDelta[ind] == 2); 
  	double cause3 = pow(exp(-frailty)*lambda3*exp(obsbmi2[ind]*Theta[8] + obsbmi3[ind]*Theta[9] + obsage[ind]*Theta[10] + obsage2[ind]*Theta[11]), obsDelta[ind] == 3); 


  	double cumcause1 =  exp(-frailty)*Lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1] + obsage[ind]*Theta[2] + obsage2[ind]*Theta[3]); 
  	double cumcause2 =  exp(-frailty)*Lambda2*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5] + obsage[ind]*Theta[6] + obsage2[ind]*Theta[7]); 
  	double cumcause3 =  exp(-frailty)*Lambda3*exp(obsbmi2[ind]*Theta[8] + obsbmi3[ind]*Theta[9] + obsage[ind]*Theta[10] + obsage2[ind]*Theta[11]); 

  	double dtimel = cause1*cause2*cause3*exp( -(cumcause1 + cumcause2 + cumcause3));

  	double with = pow( 1 - exp( - exp(-frailty)*H*exp(obsbmi2[ind]*Theta[39] + obsbmi3[ind]*Theta[40] + obsage[ind]*Theta[41] + obsage2[ind]*Theta[42])), obsU[ind] == 1);
  	double without = pow( exp(- exp(-frailty)*H*exp(obsbmi2[ind]*Theta[39] + obsbmi3[ind]*Theta[40] + obsage[ind]*Theta[41] + obsage2[ind]*Theta[42])), obsU[ind] == 0); 
       
  	// current-status data model
  	double morbl = with*without;
   
  	// individual contribution to the likelihood.
  	double marginal = exp(-pow(frailty,2) /(2*pow(exp(Theta[52]),2)) + pow(frailty,2))/sqrt(2*M_PI*pow(exp(Theta[52]),2));
