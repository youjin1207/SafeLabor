...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double cause1 = pow(exp(-frailty)*lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1] + obsage[ind]*Theta[2] + obsage2[ind]*Theta[3]), obsDelta[ind] == 1); 
  	...
  	double with = pow( 1 - exp( - exp(-frailty)*H*exp(obsbmi2[ind]*Theta[8] + obsbmi3[ind]*Theta[9] + obsage[ind]*Theta[10] + obsage2[ind]*Theta[11])), obsU[ind] == 1);
  	double without = pow( exp(- exp(-frailty)*H*exp(obsbmi2[ind]*Theta[8] + obsbmi3[ind]*Theta[9] + obsage[ind]*Theta[10] + obsage2[ind]*Theta[11])), obsU[ind] == 0); 
       
  	// current-status data model
  	double morbl = with*without;
   
  	// individual contribution to the likelihood.
  	double marginal = exp(-pow(frailty,2) /(2*pow(exp(Theta[39]),2)) + pow(frailty,2))/sqrt(2*M_PI*pow(exp(Theta[39]),2));
