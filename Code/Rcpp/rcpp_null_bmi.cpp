...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double cause1 = pow(frailty*lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1]), obsDelta[ind] == 1); 
  	double cause2 = pow(frailty*lambda2*exp(obsbmi2[ind]*Theta[2] + obsbmi3[ind]*Theta[3]), obsDelta[ind] == 2); 
  	double cause3 = pow(frailty*lambda3*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5]), obsDelta[ind] == 3); 


  	double cumcause1 =  frailty*Lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1]); 
  	double cumcause2 =  frailty*Lambda2*exp(obsbmi2[ind]*Theta[2] + obsbmi3[ind]*Theta[3]); 
  	double cumcause3 =  frailty*Lambda3*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5]); 

  	double dtimel = cause1*cause2*cause3*exp( -(cumcause1 + cumcause2 + cumcause3));

  	double with = pow( 1 - exp( - frailty*H*exp(obsbmi2[ind]*Theta[6] + obsbmi3[ind]*Theta[7])), obsU[ind] == 1);
  	double without = pow( exp(- frailty*H*exp(obsbmi2[ind]*Theta[6] + obsbmi3[ind]*Theta[7])), obsU[ind] == 0); 
       
  	// current-status data model
  	double morbl = with*without;
   
  	// individual contribution to the likelihood.
  	double marginal = exp(Theta[44])*pow(frailty, exp(Theta[44])-1)*exp(-pow(frailty,exp(Theta[44])))*exp(frailty);
