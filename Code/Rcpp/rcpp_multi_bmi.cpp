...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double cause1 = pow(frailty*lambda1*exp(obsbmi2[ind]*Theta[0] + obsbmi3[ind]*Theta[1]), obsDelta[ind] == 1); 
  	...
  	double with = pow( 1 - exp( - frailty*H*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5])), obsU[ind] == 1);
  	double without = pow( exp(- frailty*H*exp(obsbmi2[ind]*Theta[4] + obsbmi3[ind]*Theta[5])), obsU[ind] == 0); 
       
  	// current-status data model
  	double morbl = with*without;
   
  	// individual contribution to the likelihood.
  	double marginal = exp(Theta[33])*pow(frailty, exp(Theta[33])-1)*exp(-pow(frailty,exp(Theta[33])))*exp(frailty);
