...
double joint(double frailty, NumericVector Theta, int ind){
	...
	double cause1 = pow(frailty*lambda1*exp(obsage[ind]*Theta[0] + obsage2[ind]*Theta[1]), obsDelta[ind] == 1); 
  	...
  	double with = pow( 1 - exp( - frailty*H*exp(obsage[ind]*Theta[4] + obsage2[ind]*Theta[5])), obsU[ind] == 1);
  	double without = pow( exp(- frailty*H*exp(obsage[ind]*Theta[4] + obsage2[ind]*Theta[5])), obsU[ind] == 0); 
